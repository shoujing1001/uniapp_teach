<template>
	<view @touchmove="move" class="mask" :class="!show?'':'mask-show'" :style="{backgroundColor:show?maskBg:'rgba(0,0,0,0)'}" @tap="tapMask">
		<view class="popups" :class="[theme]"
			...
			<view v-for="(item,index) in popData" :key="index" @tap.stop="tapItem(item,index)" 
				class="itemChild view" :class="[direction=='row'?'solid-right':'',item.disabled?'disabledColor':'']">
				<image class="image" :src="item.icon" v-if="item.icon"></image><text 
				:class="[index == sslect ? 'text-select' : '']"
				>{{item.title}}</text> 
			</view>
			<slot></slot>
		</view>
...
<script>
	export default{
		...
		methods:{
			tapMask(){
				//zj修改 2020-1-15 点击遮盖层可关闭
				//直接修改prop值app端不可用
				//this.value = false;
				this.$emit('tapPopup',false);//改为传递此函数，参数为false时调用处只需关闭显示即可
